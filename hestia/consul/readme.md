...
- **TTL 健康检查**：基于 Consul Agent 的 TTL check 机制，注册后定期心跳保活，service 下线后自动注销。
...
```
Register
  ├── Agent.ServiceRegister（含 TTL check）
  ├── Agent.UpdateTTL("passing")                 ← 初始健康状态
  └── goroutine: 每 TTL/2 调用 Agent.UpdateTTL     ← 心跳续命

服务宕机 / keepalive 停止
  └── TTL 到期 → check 变 critical
      └── deregisterCriticalServiceAfter 到期 → 自动注销
```
...
registry, err := consul.NewRegistry(
    ...
    consul.WithDialTimeout(10*time.Second),
    consul.WithTTL("30s"),
    consul.WithDeregisterCriticalServiceAfter("90s"),
    consul.WithPrefix("hestia"),
    consul.WithToken("your-acl-token"),
    consul.WithValidateAddress(true),
)
```
...
默认情况下，`GetServices` 每次都会从 Consul 读取最新数据。如需启用本地缓存并通过 blocking query 实时刷新，可配置：
...
启用后，首次获取某服务列表时会启动 goroutine 通过 blocking query 持续监听变更，并在本地缓存中更新服务列表。

## gRPC 服务发现

`hestia/consul` 提供了 gRPC resolver，支持通过 `consul:///service_name/version` 形式的 target 直接发现服务。
...
- resolver 内部优先复用 `consulDiscovery` 的 blocking query 能力感知变更；若传入的 discovery 不是 consul 实现，则退化为 10 秒轮询。
- 服务暂时不存在时，resolver 不会直接失败，而是返回空地址列表并持续监听；待服务注册后会自动更新。
...
4. **服务注销**：`Deregister` 会取消 keepalive context，停止心跳。心跳停止后 TTL 到期，check 变 critical，`deregisterCriticalServiceAfter` 到期后 Consul 会自动清理该服务。
5. **地址解析**：注册时 `Address` 为空 host（如 `:8080`）或 `::` 时，会自动解析为本机第一个非回环 IPv4 地址。K8s 生产环境建议通过 Downward API 显式注入 Pod IP。
6. **TTL 心跳间隔**：默认 TTL 为 30 秒，心跳间隔为 TTL/2=15 秒。可通过 `WithTTL` 调整，TTL 越短则故障检测越及时，但对 Consul agent 的压力也越大。
7. **deregisterCriticalServiceAfter 设置**：默认值为 90 秒（3 倍 TTL），应确保该值大于 TTL，否则服务可能在正常运作期间被误注销。
8. **版本过滤精度**：版本号以 `hestia-version=v1` 形式存储为 Consul tag，Health API 基于 tag 精准匹配。Consul 的 tag 过滤是子集匹配，非健康实例（`passingOnly=true`）会被自动排除。
9. **并发安全**：`consulDiscovery` 内部使用读写锁保护服务列表缓存，可安全并发调用 `GetServices` 和 `Get`。
10. **错误处理**：当目标服务没有任何可用实例时，`GetServices` 会返回 `hestia.ErrServicesNotFound`。
11. **字段默认值**：注册时若 `Weight` 为 0，会自动默认设置为 100；`Healthy` 在注册成功后为 `true`，注销后为 `false`。
12. **协议类型**：`Protocol` 支持 `hestia.ProtocolGRPC` 和 `hestia.ProtocolHTTP`，gRPC resolver 会自动过滤掉非 GRPC 的实例。
13. **gRPC resolver 空列表**：服务暂时不存在时，resolver 不会直接失败，而是返回空地址列表并持续监听；待服务注册后会自动更新。
14. **endpoint 格式**：`NewRegistry` 和 `NewDiscovery` 接受 `host:port` 格式（如 `127.0.0.1:8500`），也兼容带 `http://` 前缀的格式，实现层会自动处理。
15. **与 etcd 实现的差异**：Consul 使用 Agent API 原生管理服务而非 KV 存储；使用 TTL check 而非 lease 续约；watch 使用 blocking query 而非 etcd 的 watch channel。接口层面完全兼容，业务代码无需修改即可切换注册中心。
