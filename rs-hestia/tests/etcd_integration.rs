...
//!
//! These tests are ignored by default because they require a running etcd node
//! at `127.0.0.1:12379`. Start one with:
//!
//! ```text
...
async fn test_discovery_watch() {
    ...
    let registry = new_registry(Options::new(endpoints()))
        .await
        .expect("create registry");
    let discovery = new_discovery(Options::new(endpoints()).with_enable_watched())
        .await
    ...
    // Allow watch goroutine to start.
