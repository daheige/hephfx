...
use super::discovery::EtcdDiscovery;

/// gRPC resolver builder backed by a [`Discovery`] implementation.
///
/// This is the Rust equivalent of Go's `resolver.Builder`. Because tonic does
/// not provide a global resolver registry, this builder returns a ready-to-use
/// load-balanced [`Channel`] for a target such as `etcd:///service_name/version`.
#[derive(Clone)]
pub struct EtcdResolverBuilder {
    discovery: Arc<dyn Discovery>,
    scheme: String,
}

impl EtcdResolverBuilder {
    /// Creates a new builder for the `etcd` scheme.
    ...
    pub async fn build(&self, target: &str) -> Result<Channel> {
        ...
        if let Some(ed) = self.discovery.as_any().downcast_ref::<EtcdDiscovery>() {
            let ed = ed.clone();
            ...
            tokio::spawn(async move {
                ed.watch_with_callback(&name, &version, move |services| {
                    let current = Arc::clone(&current);
                    let mut tx = tx.clone();
                    tokio::spawn(async move {
                        let mut guard = current.lock().await;
...
pub async fn build_channel(target: &str, discovery: Arc<dyn Discovery>) -> Result<Channel> {
    EtcdResolverBuilder::new(discovery).build(target).await
}

/// Creates a new etcd gRPC resolver builder.
///
/// This is the Rust equivalent of Go's `NewEtcdResolverBuilder`.
pub fn new_resolver_builder(discovery: Arc<dyn Discovery>) -> EtcdResolverBuilder {
    EtcdResolverBuilder::new(discovery)
}

/// Registers the etcd gRPC resolver.
///
/// tonic does not have a global resolver registry like Go's gRPC, so this
/// returns the configured [`EtcdResolverBuilder`] which can be used to build
/// channels directly.
pub fn register_etcd_resolver(discovery: Arc<dyn Discovery>) -> EtcdResolverBuilder {
    EtcdResolverBuilder::new(discovery)
}
...
fn parse_target(target: &str) -> Result<(String, String)> {
    ...
    if path.is_empty() {
        return Err(HestiaError::InvalidTarget(format!(
            "etcd resolver target path is empty, got: {}",
            target
        )));
    }
...
mod tests {
    ...
    fn test_parse_target_name_and_version() {
        let (name, version) = parse_target("etcd:///order_service/v1").unwrap();
    ...
    fn test_parse_target_name_only() {
        let (name, version) = parse_target("etcd:///order_service").unwrap();
        assert_eq!(name, "order_service");
        assert_eq!(version, "");
    }
    ...
    fn test_parse_target_empty_path() {
        assert!(parse_target("etcd:///").is_err());
    }
